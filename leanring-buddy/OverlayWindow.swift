        ...
        Timer.scheduledTimer(withTimeInterval: 0.03, repeats: true) { timer in
            guard currentIndex < self.fullWelcomeMessage.count else {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 2.5) {
                    ...
                    // Skip the intro video entirely and jump straight to the
                    // "press control + option" prompt so the user can start
                    // talking to Clicky right away on first launch.
                    self.companionManager.startOnboardingPromptStreamAfterWelcome()
                }
                return
            }
